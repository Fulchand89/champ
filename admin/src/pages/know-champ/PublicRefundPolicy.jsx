...
import { RotateCcw, UserCheck, Trophy } from 'lucide-react';

const PublicRefundPolicy = () => {
    ...
    <div className="min-h-screen bg-[#090b15] text-white flex flex-col font-sans overflow-x-hidden">
      ...
      <div className="relative pt-36 pb-16 bg-gradient-to-b from-[#0b0c16] via-[#100713] to-[#090b15] border-b border-gray-900 flex flex-col items-center text-center px-4">
        ...
        <h1 className="text-3xl sm:text-5xl font-black mb-4 text-white">
          Refund <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-orange-500">Policy</span>
        </h1>
        <p className="text-gray-400 max-w-xl mx-auto text-sm sm:text-base">
          Understand our guidelines regarding contest cancellations, payment reconciliation, and wallet refunds.
        </p>
      ...
      <div className="w-[calc(100%-32px)] max-w-[1100px] mx-auto px-4 sm:px-6 lg:px-8 py-12 flex-1 min-w-0">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 space-y-4">
            ...
            <p className="text-white/50 text-sm">Loading Refund Policy...</p>
          </div>
        ) : refund?.content ? (
          <div className="bg-[#0f111d] rounded-2xl border border-white/10 p-6 sm:p-10 shadow-2xl space-y-6 overflow-hidden max-w-full">
            <div className="flex flex-wrap items-center justify-between pb-6 border-b border-white/10 gap-3">
              <span className="text-xs font-semibold text-white/50">
                Version: <strong className="text-white">{refund.version || 'v1.0.0'}</strong>
              </span>
              <span className="text-xs font-semibold text-white/50">
                Last Updated:{' '}
                <strong className="text-white">
                  {refund.publishedAt ? new Date(refund.publishedAt).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }) : 'Recently'}
                </strong>
              </span>
            </div>
          ...
          <div className="text-center py-20 bg-[#0f111d] rounded-2xl border border-white/10 p-8">
            <p className="text-white/60 text-sm">Refund policy content will be published soon.</p>
          </div>
        )}
      </div>
...
export default PublicRefundPolicy;
